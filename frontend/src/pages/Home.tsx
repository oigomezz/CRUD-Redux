...
export default function Home() {
  ...
  const { loading, employeeList, error, updateState, response } =
    useAppSelector((state) => state.employeeKey);
  ...
  const updateEmployee = (item: EmployeeWithId) => {
    setId(item.id);
    setName(item.name);
        ...
        <TableContainer component={Paper} sx={{ marginTop: "16px" }}>
          <Table sx={{ minWidth: 659 }} aria-label="simple table">
            ...
            <TableBody>
              ...
              {employeeList &&
                employeeList.map((item: EmployeeWithId, index: number) => (
                  <TableRow
                    key={index}
                    sx={{
